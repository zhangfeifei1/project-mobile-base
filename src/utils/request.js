...
let uuidKey = ''
const netWorkType = '0'// 0：不走网关，1：走网关
// 请求拦截器
service.interceptors.request.use(
  config => {
    ...
    config.headers['appid'] = 1552274783265// 设置为该应用的appid
    if (netWorkType === '0') {
      // 不走网关 测试
    ...
    if (config.method === 'post') {
      ...
      if (isEncryptData(config)) {
        ...
        const dataString = qs.stringify(config.data)// 对报文数据进行加密
...
function encrptSetting(config) {
  ...
  // const symmetryEncrept = store.state.app.encrptParams.appSymmetricEncryptionType
